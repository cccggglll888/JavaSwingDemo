package com.cgl.swing.graphics;
 ...
 * 五颜六色满天星的实现
 * 
 * @author monitor Created on 2011-3-3, 22:47:03
 */
public class StarShine extends JComponent {
	...
	private static Color[][] colors = { { Color.WHITE, Color.BLACK },
			{ Color.WHITE, Color.BLUE }, { Color.ORANGE, Color.PINK },
	...
	public StarShine() {
		...
		new Timer(500, new ActionListener() {
			public void actionPerformed(ActionEvent evt) {
				...
				long um = tm - fm;
				menInfo = String.format("%d / %d MB  %d", um / (1024 * 1024),
						mm / (1024 * 1024), stars.size());
				repaint();
			}
		}).start();
	}
	...
	protected void paintComponent(Graphics g) {
		...
		// 天空背景
		GradientPaint background = new GradientPaint(0f, 0f,
				Color.GRAY.darker(), 0f, (float) getHeight(),
				Color.GRAY.brighter());
		g2d.setPaint(background);
		g2d.fillRect(0, 0, getWidth(), 4 * getHeight() / 5);
		// 地面背景
		background = new GradientPaint(0f, (float) 4 * getHeight() / 5,
				Color.BLACK, 0f, (float) getHeight(), Color.GRAY.darker());
		g2d.setPaint(background);
		...
		for (Shape star : stars) {
			...
			Point2D center = new Point2D.Float(rect.x + (float) rect.width
					/ 2.0f, rect.y + (float) rect.height / 2.0f);
	 ...
	 * @return 一个多边形
	 */
	private static Shape getStar(double x, double y, double innerRadius,
			double outerRadius, int pointsCount) {
